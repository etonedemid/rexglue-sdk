...
REXCVAR_DEFINE_STRING(occlusion_query, "fast", "GPU",
                      ...
                      " fast: Ask the GPU but don't wait; writes a cached result immediately.\n"
...
REXCVAR_DEFINE_INT32(occlusion_query_fake_lower_threshold, 80, "GPU",
                     "Lower end of the fake sample count value written on EVENT_WRITE_ZPD "
                     "when real occlusion queries are disabled. -1 writes nothing.")
    .range(-1, 100000)
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_INT32(occlusion_query_fake_upper_threshold, 100, "GPU",
                     "Upper end of the fake sample count value written on EVENT_WRITE_ZPD. "
...
ZPDMode ParseZPDMode(std::string_view value) {
  if (value == "strict") {
...
bool CommandProcessor::Initialize() {
  ...
  // Initialize the cached ZPD mode from the current CVAR value.
  SetZPDMode(ParseZPDMode(REXCVAR_GET(occlusion_query)));
...
void CommandProcessor::PrepareForWait() {
  trace_writer_.Flush();
  // Give strict ZPD a chance to retire a pending report before the guest
  // polls again.
  PumpPendingRetire();
}
...
bool CommandProcessor::ExecutePacketType3_EVENT_WRITE_ZPD(memory::RingBuffer* reader,
  ...
  auto write_batch_fake = [&]() {
    ...
    auto lower_threshold = REXCVAR_GET(occlusion_query_fake_lower_threshold);
    auto upper_threshold = REXCVAR_GET(occlusion_query_fake_upper_threshold);

    if (lower_threshold >= 0) {
      fake_zpd_sample_count_ =
          (fake_zpd_sample_count_ <=
           static_cast<uint32_t>(lower_threshold))
              ? static_cast<uint32_t>(upper_threshold)
  ...
  if (REXCVAR_GET(occlusion_query_log) && report) {
    REXLOG_INFO(
        "ZPD: EVENT_WRITE_ZPD report_address=0x{:08X} "
  ...
  // Sticky fallback — stop using normal query path and feed cumulative fake
  ...
  // Detect batched pattern and pull the fake ripcord.
  ...
  // Conventional fake fallback for records marked as pending.
  auto lower_threshold = REXCVAR_GET(occlusion_query_fake_lower_threshold);
  auto upper_threshold = REXCVAR_GET(occlusion_query_fake_upper_threshold);
  if (lower_threshold < 0 || !report_record_base || !guest_marks_end) {
    return true;
  }
