...
std::vector<uint32_t> AchievementManager::UnlockAchievements(const std::vector<uint32_t>& ids) {
  ...
  for (uint32_t id : ids) {
    ...
    // Mirror to shared gamer profile
    rex::gamer::GamerProfileManager::instance().unlock_achievement(
        kernel_state_->title_id(), static_cast<uint16_t>(id),
        achievement.label, achievement.description, achievement.gamerscore);

    // Fire unlock callback (e.g. achievement toast)
    if (unlock_callback_) {
      auto icon = GetAchievementIconPng(id);
      unlock_callback_(achievement.label, achievement.gamerscore, std::move(icon));
    }
