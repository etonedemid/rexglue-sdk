...
class AchievementManager {
 public:
  ...
  // Callback for achievement unlock notifications (name, gamerscore, icon_png).
  using UnlockCallback = std::function<void(const std::string&, uint16_t, std::vector<uint8_t>)>;
  void SetUnlockCallback(UnlockCallback cb) { unlock_callback_ = std::move(cb); }
